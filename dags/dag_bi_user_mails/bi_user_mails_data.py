...
def send_mail(email_sender: EmailSender, user_data: dict) -> None:
    ...
    email_sender.send_email(
        ...
        recipients=[user_data["email_adresse"]],
        subject=user_email_subject,
        body=user_email_body,
        attachments=[],
    )

    logger.info(f"Sent welcome email for new user: {user_data['email_adresse']}")

    if user_data.get("brugergruppe_navn") != bi_user_mail_runtime_config.get("default_user_group", "Web statistik bruger").strip():

        notification_subject, notification_body = _build_notification_email(bi_user_mail_runtime_config, user_data)

        email_sender.send_email(
            sender=bi_user_mail_runtime_config["sender_email"],
            recipients=[bi_user_mail_runtime_config["support_email"]],
