...
def set_adm_org_var():
    ...
    Variable.set("nexus_adm_org_dict", adm_org_list)
...
with DAG(
    ...
    catchup=False,
    max_active_runs=1,
    description="Set the dict of administrative organization units for Nexus by querying Delta",
    tags=['nexus', 'delta', 'permission', 'sync',],
) as dag:

    task = PythonOperator(
        task_id="dag_nexus_set_adm_org_dict",
